...
class CategoriaList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $id_categoria = new THidden('id_categoria');
        $nome = new TEntry('nome');
        $nome->forceUpperCase();

        // add the fields
        $this->form->addFields( [ new TLabel('') ], [ $id_categoria ] );
        $this->form->addFields( [ new TLabel('CATEGORIA') ], [ $nome ] );
        ...
        // $column_id_categoria = new TDataGridColumn('id_categoria', 'Idcategoria', 'right');
        $column_nome = new TDataGridColumn('nome', 'CATEGORIA', 'left');
        ...
        $action1 = new TDataGridAction(['CategoriaForm', 'onEdit'], ['id_categoria'=>'{id_categoria}']);
        $action2 = new TDataGridAction([$this, 'onDelete'], ['id_categoria'=>'{id_categoria}']);
    ...
    public function onSearch()
    {
        ...
        TSession::setValue(__CLASS__.'_filter_id_categoria',   NULL);
        TSession::setValue(__CLASS__.'_filter_nome',   NULL);

        if (isset($data->id_categoria) AND ($data->id_categoria)) {
            $filter = new TFilter('id_categoria', 'like', "%{$data->id_categoria}%"); // create the filter
            TSession::setValue(__CLASS__.'_filter_id_categoria',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if (TSession::getValue(__CLASS__.'_filter_id_categoria')) {
                $criteria->add(TSession::getValue(__CLASS__.'_filter_id_categoria')); // add the session filter
            }
